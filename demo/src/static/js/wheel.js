(function(window,document){
    ...
    function Page(_ref) {
        ...
        this.pageTotal = Math.ceil(dataTotal/pageAmount); //总共多少页
        this.pageAmount = pageAmount|| 10; //每页多少条
        ...
        this.prevText = prevText || 'Previous'; //是否支持跳转
        this.nextText = nextText || 'Next'; //是否支持跳转
        this.firstText = firstText || 'First'; //是否支持跳转
        this.lastText = lastText || 'Last'; //是否支持跳转
        if(dataTotal >=0 && this.pageTotal>=0){
            this.init();
        }else{
            console.log("总页数或者总数据不存在")
        }
    };
    // 给实例对象添加公共属性和方法
    Page.prototype = {
        ...
        showPageTotal: function showPageTotal() {
            var that = this;
            var li = document.createElement('li');
            li.innerHTML = that.curPage + '&nbsp;of&nbsp;' + that.pageTotal;
            li.className = 'totalPage page-en-style';
            this.ul.appendChild(li);
            // var li2 = document.createElement('li');
            // li2.innerHTML =  that.pageAmount + '&nbspdata/page';
            // li2.className = 'totalPage';
            // this.ul.appendChild(li2);
            var li3 = document.createElement('li');
            li3.innerHTML = 'Total&nbsp' + that.dataTotal + '&nbsp('+ that.pageAmount +'&nbsp/&nbsppage)';
            li3.className = 'totalPage page-en-style';
    ...
    //提示插件
    class Message{
        constructor(type,msg,showClose,time){
            this.type = type == null ? 'info' : type;
            this.msg = msg == null ? '提示信息' : msg;
            this.showClose = showClose == null ? false:showClose;
            this.time = time == null ? 3000:time;
        }
        info(params){
            let obj = {
                type :"info",
            ...
            let last = {};
            Object.assign(last, obj, params);
            let html;
            last.closeClass = last.showClose == true?'zx-inline':'zx-none'
            switch (last.type){
                case "info":
                    last.className = "zx-info";
                    last.icon = "xinxi";
                break;
                case "success":
                    last.className = "zx-success";
                    last.icon = "xiugaichenggong";
                break;
                case "warning":
                    last.className = "zx-warning";
                    last.icon = "jinggao";
                break;
                case "error":
                    last.className = "zx-error";
                    last.icon = "cuowu";
                break;
            }
            html = `
                <div class = "${last.className}">
                    <i>
                        <svg class = "icon" aria-hidden = "true">
                            <use xlink:href = "#${last.icon}"></use>
                        </svg>
                    </i>
                    <span>
                        ${last.msg}
                    </span>
                    <u class = "${last.closeClass}">✖</u>
                </div>
            `
            $('body').append(html);
            let scrollTop = $(window).scrollTop() + 30;
            $(`.${last.className}`).animate({top:`${scrollTop}px`},300,() =>{
                setTimeout(() => {
                    $(`.${last.className}`).animate({top:'-60px'},300,()=>{
                        $(`.${last.className}`).remove();
                    });
                },last.time)
            });
            $(`.${last.className} u`).on('click',function(){
                $(this).parent().animate({top:'-60px'},300,()=>{
                    $(this).parent().remove();
                });
            });
        }
