...
var Windy = function( params ){
	...
	const MIN_VELOCITY_INTENSITY =  0;                      // velocity at which particle intensity is minimum (m/s)
	const MAX_VELOCITY_INTENSITY =  0.5;                     // velocity at which particle intensity is maximum (m/s)
	const VELOCITY_SCALE = (0.5 || 0.005) * (Math.pow(window.devicePixelRatio,1/3) || 1); // scale for wind velocity (completely arbitrary--this value looks nice)
	const MAX_PARTICLE_AGE = 90;                         	 // max number of frames a particle is drawn before regeneration
	const PARTICLE_LINE_WIDTH =  1.5;                           // line width of a drawn particle
	const PARTICLE_MULTIPLIER = 1 / 30;            // particle count scalar (completely arbitrary--this values looks nice)
	const PARTICLE_REDUCTION =  1.6;   // multiply particle count for mobiles by this amount
	const FRAME_RATE = 15, FRAME_TIME = 1000 / FRAME_RATE;   // desired frames per second
  //var defaulColorScale = ["#2d017e", "#3c02ae", "#4201c1", "#012fdc", "#015dda","#049bcc"];
	//"#0f4ee9  #73b0ee  #098594  #ea9112  #31d4d2   #5feb86   #3cb41c];
	//  var defaulColorScale = ["#1567af","#1115bd", "#2984cb", "#1115bd", "#149bbd", "#1128bd","#0386b8","#321dd6","#0463bb","#4c3ad6"];
 var defaulColorScale = ["#1567af"];
	const colorScale = defaulColorScale;
