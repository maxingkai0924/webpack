...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  ,table = {
    config: {
        resultAttr:{},//添加返回参数对象  add by mengLei
        checkName: 'checked' //是否选中状态的字段名 add by mengLei
      ,indexName: 'LAY_TABLE_INDEX' //下标索引名
      
      //修改默认参数 add by mengLei
        ,request:{
            pageName: 'page'//page
  ...
  ,thisTable = function(){
    ...
    //添加返回参数对象  add by zqf
    var reload_fn=function (options) {
          ...
          ,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
            ,'<div class="layui-table-cell laytable-cell-'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# } }}'
              ,'{{# } }}'
            ,'" align="center">'//修改表头标题居中显示 add by mengLei
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,dataType: 'json'
         , async: false //改为同步 add by mengLei
        ,success: function(res){
              that.config.resultAttr=res.attr; //添加返回参数 add by mengLei
